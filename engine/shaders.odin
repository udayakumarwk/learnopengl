...
create_shader_from_string :: proc(vertex_string, fragment_string:cstring) -> (shader:Shader)
{
    
    shader.vertex_shader =  vertex_string
    shader.fragment_shader = fragment_string
...
update_shader_uniform1i :: proc(shader:^Shader, location:cstring, i:i32)
{
    if(!shader.isOk)
    {
        return
...
update_shader_mat4 :: proc(shader:^Shader, location:cstring, trans:^glm.mat4)
{
    if(!shader.isOk)
    {
        return
    ...
    loc := gl.GetUniformLocation(shader.program_ID, location)
    gl.UniformMatrix4fv(loc, 1, gl.FALSE, auto_cast trans)
}


load_default_shaders :: proc() -> (shader:Shader)
{
    return create_shader_from_string(DEFAULT_VERTEX_SHADER, DEFAULT_FRAGMENT_SHADER)
}
